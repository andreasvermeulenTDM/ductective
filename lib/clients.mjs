...
// ---------------------------------------------------------------------------
// Answer generation — stubbed. Provider-neutral seam.
// ---------------------------------------------------------------------------

/**
 * The exported signature is deliberately not provider-shaped: S4 requires that
 * Stage 3 replaces this function *body* without rewiring callers, and M11 puts the
 * Gemini adapter behind it in `lib/providers/gemini.mjs`, selected by
 * `LLM_PROVIDER`. Nothing above this line should learn the provider's name.
 *
 * Note `announce()` builds its banner as `${name.toUpperCase()}_API_KEY`, so the
 * stub name and the env var have to agree — 'gemini' ⇄ GEMINI_API_KEY.
 *
...
export async function complete(args = {}) {
  ...
  if (process.env.GEMINI_API_KEY) {
    ...
    if (provider !== 'gemini') {
      throw new Error(`LLM_PROVIDER="${provider}" has no adapter. Only 'gemini' is implemented.`);
    }
    // Imported lazily so a missing key path never pays for loading the adapter,
    // and so this module keeps working in environments that only use embeddings.
    const { complete: geminiComplete } = await import('./providers/gemini.mjs');
    return geminiComplete(args);
  }

  if (!STUBS_ALLOWED) {
    throw new Error(
      'GEMINI_API_KEY is empty. Set it, or pass ALLOW_STUBS=true to run against the stub.'
    );
  }
  announce('gemini');
  ...
  return {
    text:
      `[STUB RESPONSE — not generated by any model, contains no real diagnostic guidance]\n` +
