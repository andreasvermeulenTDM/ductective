 ...
 */
/**
 * Validate an inbound photo and get it down to a size worth sending.
 *
 * Extracted from `identifyUnit` when `/diagnose` gained photo turns (ST-17), so both
 * routes run the *same* checks. A second copy of image validation is exactly where a
 * gap opens: the decode guard, the JPEG-only rule and the size ceiling all have to
 * hold on whichever route an attacker reaches first.
 *
 * @returns {{sent:{data:Buffer,width:number,height:number}, original:{width:number,height:number,bytes:number}, resized:boolean}}
 */
export function prepareImage(image, mimeType = 'image/jpeg') {
  ...
  return {
    sent,
    original: { width: dims.width, height: dims.height, bytes: buf.length },
...
export async function identifyUnit(
  ...
  const { sent, original } = prepareImage(image, mimeType);
  ...
  return {
    ...
    meta: {
      ...
      image: {
        ...
        width: sent.width,
        height: sent.height,
        resized: sent.data.length !== original.bytes,
      },
    },
  };
}
