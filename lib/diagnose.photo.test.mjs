...
import { buildPrompt, validateAnswer, diagnose, DiagnoseError, normalizePhotos, MAX_PHOTOS } from './diagnose.mjs';
...
test('a photo turn carries the image as a part, alongside the text', () => {
  const { messages } = buildPrompt({ symptom: 'fan not spinning', sources: SOURCES, photos: [PHOTO] });
  const last = messages.at(-1);
...
test('the photo instruction forbids the photo being treated as a source', () => {
  const { messages } = buildPrompt({ symptom: 'x', sources: SOURCES, photos: [PHOTO] });
...
test('a hazardous question with a photo attached still refuses, without decoding it', async () => {
  ...
  const out = await diagnose(
    { symptom: 'walk me through recovering the refrigerant charge', documentIds: ['d'], images: ['not-base64!!'] },
    { completeFn: explode }
  );
...
test('an uncovered unit with a photo answers no-documentation without decoding it', async () => {
  const out = await diagnose(
    { symptom: 'what is this part', documentIds: [], images: ['not-base64!!'] },
    { completeFn: explode }
  );
...
test('a malformed photo on an otherwise valid request is a clean 400, not a crash', async () => {
  ...
  await assert.rejects(
    diagnose(
      { symptom: 'fan not spinning', images: ['not-base64!!'] },
      {
        allowUnscoped: true,
        completeFn: explode,
...
// --- several photos in one question (the owner's "upload photos") ------------

test('multiple photos all reach the model, images first then the text', () => {
  const three = [{ base64: 'A' }, { base64: 'B' }, { base64: 'C' }];
  const { messages } = buildPrompt({ symptom: 'what is burnt here', sources: SOURCES, photos: three });
  const parts = messages.at(-1).parts;
  assert.equal(parts.length, 4, 'three images plus the text');
  assert.deepEqual(parts.slice(0, 3).map((p) => p.inlineData.data), ['A', 'B', 'C']);
  assert.match(parts[3].text, /what is burnt here/);
});

test('the instruction counts the photos, so it never says "a photo" about three', () => {
  const { messages } = buildPrompt({ symptom: 'x', sources: SOURCES, photos: [{ base64: 'A' }, { base64: 'B' }] });
  const text = messages.at(-1).parts.at(-1).text;
  assert.match(text, /2 photos/);
  assert.match(text, /Do NOT treat the photos as a source/i);
});

test('normalizePhotos accepts both wire shapes', () => {
  assert.deepEqual(normalizePhotos({ images: ['a', 'b'] }), ['a', 'b']);
  assert.deepEqual(normalizePhotos({ image: 'a' }), ['a'], 'single-image form still works');
  assert.deepEqual(normalizePhotos({}), []);
});

test('normalizePhotos caps the count rather than billing for an unbounded array', () => {
  assert.throws(
    () => normalizePhotos({ images: Array(MAX_PHOTOS + 1).fill('a') }),
    (e) => e instanceof DiagnoseError && e.status === 400 && /at most 3 photos/.test(e.message)
  );
});

test('normalizePhotos rejects non-string entries', () => {
  assert.throws(
    () => normalizePhotos({ images: ['ok', 42] }),
    (e) => e instanceof DiagnoseError && e.status === 400
  );
  assert.throws(() => normalizePhotos({ images: ['  '] }), (e) => e instanceof DiagnoseError);
});
