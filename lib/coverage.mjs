...
/**
 * E-1 (Stage 5.5, round 4) — the withhold for a unit that resolved to **zero**
 * documents.
 *
 * ## The defect this closes
 *
 * `scopedNoDocumentationBody` above fixed the withhold for a unit with a scope.
 * It is reached from `lib/diagnose.mjs` only when `scopeRows.length` is non-zero,
 * and the empty-scope branch returns nineteen lines earlier — so a unit that
 * resolved to no documents still received the unscoped `NO_DOCUMENTATION`
 * constant, which asks *"if you tell me the make and model off the nameplate"*
 * of a technician who had just sent `equipment: "Rheem RKNL-B073CL"` on the same
 * request. Measured by the round's own eval on four adjacent units. Verbatim the
 * defect `02-user-stories-round4.md` §2.2 names as *(i)*: asking for something
 * already given.
 *
 * ## What is different here from the scoped case, and why the words differ
 *
 * On the scoped path the claim "none of them cover this unit" was **false** —
 * there were 33 documents. Here it is **true**: the unit resolved to nothing, so
 * saying so plainly is the honest answer and is kept. The only defect was the
 * request for a nameplate already supplied, and the terminal shape.
 *
 * ## The rules, the same three that bind every body in this file
 *
 *  - no "nameplate", because it has already been given;
 *  - the manufacturer names are lifted from `documents` rows via
 *    `coveredFamilies` — there is no literal here either, so this cannot promise
 *    coverage the corpus does not have (hard constraint 2);
 *  - it ends in a question, so the turn continues rather than dead-ends, and it
 *    carries no numbered or bulleted line — `refusalLeaksProcedure` reads those
 *    as the shape a leaked procedure takes, and `diagnose()` runs that check over
 *    this body before emitting it.
 *
 * `meta.noDocumentation` stays `true` on this path. Only the words change, which
 * is what keeps ST-R01's log honest.
 *
 * @param {Array<{manufacturer?: string}>} families — `coveredFamilies` output, or []
 */
export function unresolvedUnitBody(families = []) {
  const opening =
    'I checked, and I don’t hold a manual for that unit.\n\n' +
    'I answer only from manufacturers’ own documentation, so rather than work ' +
    'from a manual for a different machine, I’d rather tell you I have nothing ' +
    'for this one.';

  const names = (families ?? []).map((f) => f?.manufacturer).filter(Boolean);
  if (!names.length) {
    // The database read failed or the corpus is empty. Still truthful, still
    // continues, and it never invents a list — the same degradation
    // `capabilityBody`'s third step takes, for the same reason.
    return `${opening}\n\nWhich unit should I look at instead?`;
  }
  ...
  return (
    `${opening}\n\n` +
    `What I do hold is ${list}. ` +
    'Is this one of those under a different model designation, or is there another ' +
    'unit on the job I can help with?'
  );
}
