...
/**
 * Automatic model fallback — owner request, 10 Aug 2026.
 *
 * Free-tier quota is per MODEL per day (measured:
 * GenerateRequestsPerDayPerProjectPerModel-FreeTier), so when the pinned model's
 * bucket empties the chain tries the next. Defaults are concrete models verified
 * CALLABLE on this account (listed ≠ callable — probed 4 and 10 Aug), quality-
 * ordered: full Flash first, then the lite tiers. Override with
 * GEMINI_MODEL_CHAIN=comma,separated,models.
 *
 * No `-latest` aliases in the chain, deliberately: probing showed
 * gemini-flash-lite-latest answers as modelVersion gemini-3.5-flash-lite — an
 * alias resolves to a concrete model and therefore shares its quota bucket, so a
 * chain entry for it would re-probe a bucket that just 429'd and burn a request
 * doing it.
 *
 * Two rules keep this compatible with the eval charter's pinned-model discipline:
 *  - fallback is LOUD: results carry `fallback: true` and the model that actually
 *    answered, and callers surface both in meta — a transcript can never pass as
 *    the pinned model when it was not.
 *  - an EXPLICIT `model` argument never chains. Only the default path falls back,
 *    so a comparison run (M12 --model, ST-16's pinned rounds) measures exactly
 *    the model it names or fails trying.
 */
export const MODEL_CHAIN = (process.env.GEMINI_MODEL_CHAIN
  ? process.env.GEMINI_MODEL_CHAIN.split(',').map((m) => m.trim()).filter(Boolean)
  : [DEFAULT_MODEL, 'gemini-3.5-flash', 'gemini-3.5-flash-lite', 'gemini-3.1-flash-lite']
).filter((m, i, a) => a.indexOf(m) === i);

/** Models whose DAILY bucket is spent, by local day — skipped without a probe. */
const exhaustedOn = new Map();
const today = () => new Date().toDateString();
export const _resetModelMemo = () => exhaustedOn.clear();

/**
 * Daily exhaustion vs momentary throttle, from the violation detail. A PerMinute
 * 429 means try another model NOW but the bucket refills — memoizing it for the
 * day would burn a whole model's allowance on one busy minute.
 */
const quotaKind = (e) => {
  if (e?.status !== 429 || !/RESOURCE_EXHAUSTED|quota/i.test(e?.message ?? '')) return null;
  return /PerDay/i.test(e.message) ? 'day' : 'moment';
};
...
/**
 * Returns `{json, attempts}` rather than the bare body: ST-09 needs the retry
 * count in the serve log, and the attempt number is only knowable here. On
 * failure the thrown ProviderError carries `.attempts` for the same reason —
 * a failed call still consumed that many requests of the daily quota.
 */
async function post(path, body, apiKey) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    if (res.ok) return { json: await res.json(), attempts: attempt };
 ...
 *                    blockReason: string|null, attempts: number}>}
 */
export async function complete({ model, ...args } = {}) {
  // Explicit model → exactly that model, no chain (comparison runs must measure
  // what they name). Unset → the fallback chain.
  if (model) return completeWithModel({ ...args, model });

  const day = today();
  let lastErr = null;
  for (const m of MODEL_CHAIN) {
    if (exhaustedOn.get(m) === day) continue;
    try {
      const r = await completeWithModel({ ...args, model: m });
      return { ...r, requestedModel: MODEL_CHAIN[0], fallback: m !== MODEL_CHAIN[0] };
    } catch (e) {
      const kind = quotaKind(e);
      if (!kind) throw e;
      if (kind === 'day') exhaustedOn.set(m, day);
      lastErr = e;
    }
  }
  const err = new ProviderError(
    429,
    `daily quota exhausted across the model chain [${MODEL_CHAIN.join(', ')}] — resets at midnight PT`
  );
  err.attempts = lastErr?.attempts ?? 1;
  err.chainExhausted = true;
  throw err;
}
...
async function completeWithModel({
  ...
  const { json: res, attempts } = await post(`/models/${model}:generateContent`, body, apiKey);
  ...
  return {
    ...
    usage: {
      ...
      // ST-09 (M15): the implicit-cache hit count, previously discarded. Zero
      // when the prefix missed the cache; also absent (⇒ 0) from responses of
      // models without implicit caching. Kept under Google's own field name so
      // a transcript can be checked against raw usageMetadata verbatim.
      cachedContentTokenCount: res?.usageMetadata?.cachedContentTokenCount ?? 0,
    },
    finishReason,
    blocked,
    blockReason: reason,
