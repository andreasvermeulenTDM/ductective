...
const CATEGORIES = [
  {
    ...
    domain: [
      ...
      // Installation vocabulary. Absent until 16 Aug 2026 because these lists were
      // written against a diagnostic corpus, and "walk me through landing the gas
      // piping" therefore matched PROCEDURAL with no domain noun to pair it with —
      // a live leak of exactly the work this gate exists to refuse. Found by
      // probing the owner's N2 request rather than by any test.
      /\bgas (pip(e|ing)|line|train)\b/i, /\bgas connection\b/i,
    ],
  },
  {
    id: 'live_electrical',
    ...
    domain: [
      ...
      // Same gap, same date, same cause as the gas additions above: the point at
      // which a conductor is actually attached had no noun in this list, so
      // "how do I torque the lugs" and "how do I wire the disconnect switch"
      // both reached the model.
      /\blugs?\b/i, /\bterminal (block|strip|s)\b/i, /\bdisconnect switch\b/i,
      /\bterminate|termination\b/i,
      /*
       * The conductor itself, added 16 Aug 2026 after a second leak in the same
       * area. The action list already carried
       * `\bland(ing)? .{0,16}(wire|conductor)s?\b`, but sixteen characters is a
       * guess about how someone phrases it: "land the conductors" refuses while
       * "land the **line-voltage** conductors" does not, because that qualifier
       * is seventeen. Three of five real installation phrasings slipped through.
       *
       * Widening the window would only move the guess. A domain noun does not
       * guess: it refuses whenever PROCEDURAL is also present, however the
       * sentence is built — and stays silent on "what gauge conductor does this
       * need", which carries no procedural intent at all.
       */
      /\bconductors?\b/i, /\bwires?\b/i,
    ],
  },
  {
    id: 'refrigerant',
