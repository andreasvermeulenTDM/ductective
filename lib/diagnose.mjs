...
/**
 * Prefer hybrid (RRF) retrieval when the database has it, fall back to vector-only.
 *
 * `match_chunks_hybrid` ships on an unmerged branch at time of writing. Detecting
 * it at runtime means this works either way and picks up the better ranking the
 * moment that migration lands, with no code change here.
 */
export async function retrieve(symptom, { topK = TOP_K, db = supabaseAdmin() } = {}) {
  ...
  let { data, error } = await db.rpc('match_chunks_hybrid', {
  ...
  if (error) {
    // 42883 = undefined_function. Anything else is a real failure worth surfacing.
    if (!/42883|does not exist|could not find/i.test(`${error.code} ${error.message}`)) {
...
export async function diagnose({ symptom, equipment, history = [], topK = TOP_K } = {}) {
  ...
  if (hazard) {
    ...
    return {
      ...
      body,
      citations: [],
      meta: { category: hazard.category, trigger: hazard.trigger, latencyMs: Date.now() - started, model: null },
    };
  }
  ...
  if (!sources.length) {
    return {
      ...
      citations: [],
      meta: { retrieved: 0, noDocumentation: true, latencyMs: Date.now() - started, model: null },
    };
  }
