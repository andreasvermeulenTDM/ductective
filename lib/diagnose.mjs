 ...
 * @returns {Promise<{chunks: Array, scopeFallback: boolean}>}
 */
export async function retrieve(symptom, { topK = TOP_K, mode = MODE, db = supabaseAdmin(), documentIds, embedFn = embed } = {}) {
  ...
  const { embeddings } = await embedFn([symptom], { inputType: 'query' });
...
export async function diagnose(
  { symptom, equipment, history = [], documentIds, topK = TOP_K } = {},
  ...
  if (!scoped && !hasEquipment && !allowUnscoped) {
    return {
      ...
      body: UNIT_REQUIRED,
      citations: [],
      meta: { retrieved: 0, dropped: 0, noDocumentation: false, mode: MODE, latencyMs: Date.now() - started, model: null },
    };
  }
  ...
  if (scoped && documentIds.length === 0) {
    return {
      ...
      body: NO_DOCUMENTATION,
      citations: [],
      meta: { retrieved: 0, dropped: 0, noDocumentation: true, scopedTo: 0, mode: MODE, latencyMs: Date.now() - started, model: null },
    };
  }
  ...
  const { chunks, scopeFallback } = await retrieve(symptom, { topK, db, documentIds: scoped ? documentIds : undefined, embedFn });
  ...
  if (!sources.length) {
    return {
      ...
      body: NO_DOCUMENTATION,
      citations: [],
      meta: { retrieved: 0, dropped: 0, noDocumentation: true, ...scopeMeta, mode: MODE, latencyMs: Date.now() - started, model: null },
    };
  }
