...
import {
  summariseScope, scopedNoDocumentationBody, capabilityBody, unresolvedUnitBody,
} from './coverage.mjs';
...
async function readCoverageFacts(documentIds, db) {
  ...
  try {
     ...
     *
     * `supabaseAdmin()` throws when env is absent, and building the client on the
     * line above meant that throw escaped a function whose contract — three lines
     * up — is "every failure degrades to the unscoped body, never to an error".
     * The capability answer is the one question that must never come back as a
     * transport failure, and on a machine with no `.env` it did.
     *
     * Found when E-1 routed the empty-scope withhold through here and three
     * offline tests that pass no `db` started throwing. They were right to.
     */
    const client = db ?? supabaseAdmin();
    if (scoped) {
      const { data, error } = await client.from('documents').select('id, doc_type, in_scope').in('id', documentIds);
...
export async function diagnose(
  ...
  //
  // E-1 (Stage 5.5): the body is no longer `NO_DOCUMENTATION`. That constant
  // asks for a nameplate, and this branch is reached *because* a unit was named
  // and resolved to nothing — so it asked for something already given. ST-R09
  // fixed the sibling path (1e below) and could not fix this one, because that
  // fix keys on `scopeRows.length` and this returns before the rows are read.
  //
  // The select costs one round trip on a path that otherwise spends nothing.
  // Accepted deliberately: `readCoverageFacts` degrades every failure to
  // `families: []` rather than throwing, and `unresolvedUnitBody` degrades that
  // to a shorter body — so the worst case is a technician reading one sentence
  // fewer, never an error on the turn where they have already been told no.
  if (scoped && documentIds.length === 0) {
    const { families } = await readCoverageFacts(documentIds, db);
    return {
      kind: 'answer',
      body: unresolvedUnitBody(families),
      citations: [],
      meta: {
