...
export function buildPrompt({ symptom, equipment, sources, history = [], photo = null }) {
  ...
  const photoNote = photo
    ? '\n\nThe technician has attached a photo of what they are looking at. Use it to ' +
      'understand the symptom and to tell them what you can see. Do NOT treat the ' +
      'photo as a source: every numbered step must still come from, and cite, one of ' +
      'the numbered sources above. If the photo shows something the sources do not ' +
      'cover, say so plainly rather than inferring a procedure from the image.'
    : '';
  ...
  // A turn carries `parts` only when there is an image; the string form stays the
  // common path so nothing about the text-only prompt changes shape.
  const turn = photo
    ? { role: 'user', parts: [imagePart(photo.base64, 'image/jpeg'), { text: user }] }
 ...
 * @returns {Promise<{kind:'answer'|'clarify'|'refusal'|'unit_required', body:string,
 *                    citations:Array<{source_document:string,page:number,claim:string,ordinal:number}>,
 *                    meta:object}>}
 */
export async function diagnose(
  { symptom, equipment, history = [], documentIds, topK = TOP_K, image, mimeType } = {},
  ...
  if (image) {
    ...
    const { sent } = prepareImage(image, mimeType ?? 'image/jpeg');
    photo = { base64: Buffer.from(sent.data).toString('base64'), width: sent.width, height: sent.height };
  }

  // 4. Generate. Uses the sanitized history — the raw wire array never reaches the
  // model's turn list.
  const { system, messages } = buildPrompt({ symptom, equipment, sources, history: cleanHistory, photo });
