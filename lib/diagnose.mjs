...
export async function retrieve(symptom, { topK = TOP_K, mode = MODE, db = supabaseAdmin() } = {}) {
  ...
  const { data, error } = mode === 'hybrid'
    ? await db.rpc('match_chunks_hybrid', { query_embedding: vector, query_text: symptom, match_count: topK })
    : await db.rpc('match_chunks', { query_embedding: vector, match_count: topK });
