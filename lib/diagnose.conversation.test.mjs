...
test('AC 8 — RESPONSE_SCHEMA\'s kind enum is not extended', () => {
  const schema = SOURCE.slice(SOURCE.indexOf('export const RESPONSE_SCHEMA'), SOURCE.indexOf('// Validation'));
  assert.match(schema, /enum: \['answer', 'clarify', 'no_documentation'\]/);
  assert.equal(/conversational/.test(schema), false);
});
