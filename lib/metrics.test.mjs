...
// ---------------------------------------------------------------------------
// ST-R01 (D2) — an honest withhold and an uncited answer are different rows
// ---------------------------------------------------------------------------

test('diagnoseLogFields is present on every shape — absent must never read as false', () => {
  // The four shapes the log has to tell apart, plus the two gate shapes.
  const shapes = [
    { kind: 'answer', citations: [{}, {}], meta: { noDocumentation: false, scopedTo: 33 } },
    { kind: 'answer', citations: [], meta: { noDocumentation: true, scopedTo: 33 } },
    { kind: 'refusal', citations: [], meta: { category: 'refrigerant', noDocumentation: false } },
    { kind: 'conversational', citations: [], meta: { intent: 'greeting', noDocumentation: false } },
    { kind: 'unit_required', citations: [], meta: { noDocumentation: false } },
    { kind: 'clarify', citations: [], meta: { noDocumentation: false, scopedTo: 4 } },
  ];
  for (const s of shapes) {
    const f = diagnoseLogFields(s);
    assert.equal(typeof f.noDocumentation, 'boolean', `${s.kind} lost noDocumentation`);
    assert.equal(typeof f.cites, 'number', `${s.kind} lost cites`);
    assert.equal(f.kind, s.kind);
  }
});

test('diagnoseLogFields reads noDocumentation strictly — a missing meta is false, not absent', () => {
  assert.deepEqual(diagnoseLogFields({ kind: 'answer', citations: [] }), {
    kind: 'answer', noDocumentation: false, cites: 0,
  });
  // Truthiness is not enough: only an explicit `true` is a withhold.
  assert.equal(diagnoseLogFields({ meta: { noDocumentation: 'yes' } }).noDocumentation, false);
});

test('the console line gains `nodoc` on a withhold and only on a withhold', () => {
  const withhold = diagnoseLogLine({
    kind: 'answer', citations: [],
    meta: { latencyMs: 6243, retrieved: 8, noDocumentation: true, scopedTo: 33 },
  });
  assert.match(withhold, /nodoc/);
  // The exact shape the brief quotes, now readable at a glance.
  assert.match(withhold, /^answer\s+6243ms\s+retrieved=8 cites=0 nodoc scope=33$/);

  const cited = diagnoseLogLine({
    kind: 'answer', citations: [{}, {}, {}],
    meta: { latencyMs: 4355, retrieved: 8, noDocumentation: false, scopedTo: 33 },
  });
  assert.doesNotMatch(cited, /nodoc/);
  assert.match(cited, /cites=3/);
});

test('the four outcome signatures are distinct, and the fifth is the alarm', () => {
  const row = (kind, noDocumentation, cites) => ({ kind, noDocumentation, cites });
  assert.equal(classifyLogOutcome(row('answer', false, 3)), 'answers');
  assert.equal(classifyLogOutcome(row('answer', true, 0)), 'withholds');
  assert.equal(classifyLogOutcome(row('refusal', false, 0)), 'refusals');
  assert.equal(classifyLogOutcome(row('conversational', false, 0)), 'conversational');
  assert.equal(classifyLogOutcome(row('clarify', false, 0)), 'clarifies');
  assert.equal(classifyLogOutcome(row('unit_required', false, 0)), 'unitRequired');
  // Impossible by construction; counted separately so it cannot hide among answers.
  assert.equal(classifyLogOutcome(row('answer', false, 0)), 'uncitedAnswers');
});

test('a pre-ST-R01 row is classified as nothing at all — never as a false withhold', () => {
  // AC 6. The session that motivated this round is in the old format; a
  // classifier that defaulted the missing field would relabel four honest
  // withholds as uncited answers and invent the alarm it exists to raise.
  const { entries } = parseRequestLog(FIXTURE_LOG);
  const o = summarizeOutcomes(entries);
  assert.equal(o.unclassified, entries.length);
  for (const k of OUTCOME_KEYS) assert.equal(o[k], 0, `${k} counted an old-format row`);
  assert.equal(classifyLogOutcome({ kind: 'answer', cites: 0 }), null, 'noDocumentation absent');
  assert.equal(classifyLogOutcome({ kind: 'answer', noDocumentation: false }), null, 'cites absent');
});

test('summarize carries the outcome tally beside the cost numbers', () => {
  const rows = [
    { kind: 'answer', noDocumentation: false, cites: 2, latencyMs: 5000 },
    { kind: 'answer', noDocumentation: true, cites: 0, latencyMs: 6000 },
    { kind: 'refusal', noDocumentation: false, cites: 0, latencyMs: 1 },
    { kind: 'conversational', noDocumentation: false, cites: 0, latencyMs: 1 },
    { kind: 'identify', latencyMs: 20000 },
  ];
  const s = summarize(rows);
  assert.equal(s.outcomes.answers, 1);
  assert.equal(s.outcomes.withholds, 1);
  assert.equal(s.outcomes.refusals, 1);
  assert.equal(s.outcomes.conversational, 1);
  assert.equal(s.outcomes.uncitedAnswers, 0);
  assert.equal(s.outcomes.unclassified, 1, 'the /identify-unit row carries no outcome');
});

test("the owner's four-turn session, replayed, says which turns were honest withholds", () => {
  // The brief's own log lines, now carrying the two new fields. Three of them
  // were honest withholds; the clarify was not. Before ST-R01 all four read
  // identically as `cites=0`.
  const session = [
    { kind: 'answer', noDocumentation: true, cites: 0, latencyMs: 6243, scopedTo: 33 },
    { kind: 'clarify', noDocumentation: false, cites: 0, latencyMs: 9458, scopedTo: 33 },
    { kind: 'answer', noDocumentation: true, cites: 0, latencyMs: 4355, scopedTo: 33 },
    { kind: 'answer', noDocumentation: true, cites: 0, latencyMs: 7678, scopedTo: 33 },
  ];
  const o = summarizeOutcomes(session);
  assert.equal(o.withholds, 3);
  assert.equal(o.clarifies, 1);
  assert.equal(o.answers, 0);
  assert.equal(o.uncitedAnswers, 0, 'none of them was an uncited answer — they were honest');
});
