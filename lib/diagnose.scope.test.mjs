...
test('documentIds: [] short-circuits to no-documentation before any RPC or model call', async () => {
  ...
  assert.match(out.body, /don’t hold a manual for that unit/i);
  /*
   * E-1 — the assertion that would have caught the defect, rather than the copy
   * check that did not.
   *
   * This branch is reached *because* a unit was named and resolved to zero
   * documents, so asking for the nameplate asks for something already given.
   * The eval measured it on four adjacent units (Rheem, AAON, Reznor, Trane
   * Centravac) while the string check above was green on the old copy.
   */
  assert.doesNotMatch(out.body, /nameplate/i);
  assert.match(out.body, /\?\s*$/);            // continues the turn, never dead-ends
