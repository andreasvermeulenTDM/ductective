...
// The format actually issued to this project. Before this rule existed the
// scanner matched neither our live key nor anything shaped like it — the one key
// it most needed to see was invisible to it.
test('the newer AQ. Google key format is caught by shape', () => {
  const hits = scanText('GEMINI_API_KEY=AQ.Ab8RN6J-fakefakefakefakefakefake', {});
  assert.deepEqual(hits, [{ kind: 'shape', name: 'Google AI Studio key (AQ format)' }]);
});

test('prose beginning "AQ. " is not mistaken for a key', () => {
  assert.deepEqual(scanText('See appendix AQ. Then read the next section.', {}), []);
});
