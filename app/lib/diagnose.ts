...
export async function requestDiagnosis(
  ...
  /**
   * ST-17 — base64 JPEGs of what the technician is looking at, not the plate. The
   * server treats them as observations: they may inform what the answer says it can
   * see, and they can never be the source a step cites.
   */
  photos?: string[] | null
  ...
  try {
    const res = await fetch(`${BASE}/diagnose`, {
      ...
      body: JSON.stringify({
        ...
        mimeType: photos?.length ? 'image/jpeg' : undefined,
      }),
